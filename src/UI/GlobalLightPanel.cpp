...
void omp::GlobalLightPanel::renderUI(float DeltaTime)
{
    ...
    if (!m_LightRef.expired())
    {
        ...
        ImGui::DragFloat("Ambient str", &m_LightRef.lock()->m_Ambient[3], 0.1, 0.0f, 0.0f, "%.2f", 0);

        ImGui::DragFloat3("Diffusive", &m_LightRef.lock()->m_Diffuse[0], 0.1f, 0.0f, 0.0f, "%.2f", 0);
        ImGui::DragFloat("Diffusive str", &m_LightRef.lock()->m_Diffuse[3], 0.1, 0.0f, 0.0f, "%.2f", 0);

        ImGui::DragFloat3("Specular", &m_LightRef.lock()->m_Specular[0], 0.1f, 0.0f, 0.0f, "%.2f", 0);
        ImGui::DragFloat("Specular str", &m_LightRef.lock()->m_Specular[3], 0.1, 0.0f, 0.0f, "%.2f", 0);
