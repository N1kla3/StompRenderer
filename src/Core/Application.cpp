...
void omp::Application::debug_createSceneManually()
{
...

    {
        ...
        omp::AssetHandle texture_handle = m_AssetManager->createAsset("container",
                                                                      "../assets/texture.json",
                                                                      "TextureSrc");
        ...
        omp::AssetHandle spec_texture_handle = m_AssetManager->createAsset(
                "container_specular",
                "../assets/texture_specular.json",
                "TextureSrc");
        ...
        omp::AssetHandle material_handle = m_AssetManager->createAsset("def_mat",
                                                                       "../assets/def_material.json",
                                                                       "Material");
    ...
    omp::AssetHandle vik_texture_handle = m_AssetManager->createAsset("viking_texture",
                                                                      "../assets/viking_texture.json",
                                                                      "TextureSrc");
    ...
    omp::AssetHandle vik_material_handle = m_AssetManager->createAsset("vik_mat",
                                                                       "../assets/vik_material.json",
                                                                       "Material");
    ...
    omp::AssetHandle vik_model_handle = m_AssetManager->createAsset("viking_model",
                                                                    "../assets/viking_model.json",
                                                                    "Model");
    ...
    std::shared_ptr<omp::ModelInstance> viking_inst = std::make_shared<omp::ModelInstance>(
            viking_model,
            viking_material);
    ...
    // GRASS
    {
        omp::AssetHandle texture_handle = m_AssetManager->createAsset("grass",
                                                                      "../assets/grass_texture.json",
                                                                      "TextureSrc");
        auto texture = m_AssetManager->getAsset(texture_handle).lock()->getObjectAs<omp::TextureSrc>();
        if (texture)
        {
        ...
        omp::AssetHandle material_handle = m_AssetManager->createAsset("grass_mat",
                                                                       "../assets/grass_material.json",
                                                                       "Material");
        auto material = m_AssetManager->getAsset(material_handle).lock()->getObjectAs<omp::Material>();
        material->addSpecularTexture(texture);
    ...
    // WINDOW
    {
        omp::AssetHandle texture_handle = m_AssetManager->createAsset("window_texture",
                                                                      "../assets/window_texture.json",
                                                                      "TextureSrc");
        auto texture = m_AssetManager->getAsset(texture_handle).lock()->getObjectAs<omp::TextureSrc>();
        if (texture)
        {
        ...
        omp::AssetHandle model_handle = m_AssetManager->createAsset("window_model",
                                                                    "../assets/window_model.json",
                                                                    "Model");
        auto model = m_AssetManager->getAsset(model_handle).lock()->getObjectAs<omp::Model>();
        if (model)
        {
        ...
        omp::AssetHandle material_handle = m_AssetManager->createAsset("window_mat",
                                                                       "../assets/window_material.json",
                                                                       "Material");
        auto material = m_AssetManager->getAsset(material_handle).lock()->getObjectAs<omp::Material>();
        material->addSpecularTexture(texture);
        material->addTexture(texture);
    ...
    // PLANE
    {
        omp::AssetHandle texture_handle = m_AssetManager->createAsset("plane_texture",
                                                                      "../assets/plane_texture.json",
                                                                      "TextureSrc");
        auto texture = m_AssetManager->getAsset(texture_handle).lock()->getObjectAs<omp::TextureSrc>();
        if (texture)
        {
        ...
        omp::AssetHandle model_handle = m_AssetManager->createAsset("plane_model",
                                                                    "../assets/plane_model.json",
                                                                    "Model");
        auto model = m_AssetManager->getAsset(model_handle).lock()->getObjectAs<omp::Model>();
        if (model)
        {
        ...
        omp::AssetHandle material_handle = m_AssetManager->createAsset("plane_mat",
                                                                       "../assets/plane_material.json",
                                                                       "Material");
        auto material = m_AssetManager->getAsset(material_handle).lock()->getObjectAs<omp::Material>();
        material->addSpecularTexture(texture);
        material->addTexture(texture);
...
void omp::Application::debug_addLightToScene()
{
    ...
    std::unique_ptr<LightBase>&& global_one = std::make_unique<omp::LightObject<omp::GlobalLight>>(
            "global",
            std::make_shared<omp::ModelInstance>(model, material));
